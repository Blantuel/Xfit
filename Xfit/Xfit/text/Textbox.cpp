...
void TextBox::PrepareDrawEdit(const wchar_t* _text, const Font* font, unsigned _pixelSize, unsigned _color) {
	const FT_Face& face = font->face;
	...
	for (size_t i = 0; i < len; i++) {
		...
		if ((advanceX + charImages[i].advanceX) > width) {
			totalHeight += lineHeight;
			advanceX = 0;
		}
		for (int y = 0; y < charImages[i].height; y++) {
			for (int x = 0; x < charImages[i].width; x++) {
				outBitmap[(int)width * (top - charImages[i].top + y + totalHeight) + advanceX + charImages[i].left + x] = ((unsigned)charImages[i].bitmap[charImages[i].width * y + x] << 24) | _color;
			}
		}
		advanceX += charImages[i].advanceX;
	}
	...
	const RectF rect = { -1.f,1.f,1.f,-1.f };
	BuildEdit(outBitmap, width, height);
